...
Há algumas omissões relevantes. Não há tipos tupla ou recursivos. Mesmo que $PCL_{back}$ pudesse representar eles, eles complicam o \emph{Borrow Checker} e o passo de compilação. Entretanto, a omissão é principalmente por limite de tempo e escopo do projeto. Essa é a mesma razão pela omissão do operador $\KW{while}$ e declarações globais. Ainda assim pode-se gerar programas bem complexos, como o porte do Programa \ref{lst:pclback:spacial} em $PCL_{back}$ para $PCL_{front}$, descrito no Programa \ref{lst:pclfront:spacial}. Essa nova versão ainda é suscetível a erros espaciais, pois esse não é o tipo de falha em foque para $PCL_{front}$ evitar.
...
A linguagem $PCL_{front}$ foi implementada como uma AST, função de checagem de tipos dessa e função de compilação dessa a AST de $PCL_{back}$. Dessa forma, todas as regras apresentadas para o \emph{Borrow Checker} de $PCL_{front}$ foram implementadas como funções Haskell. Mesmo sem ser possível atingir a etapa de prova desse sistema, vários exemplos foram gerados para testar o \emph{Borrow Checker} e validar que esse se comporta como esperado. Dos testes gerados, a lógica do sistema se mantém. Faltou-se ainda cavar exceções para a região estática do programa ($0$) e testar essa inclusão.

O código se encontra hospedado junto ao de $PCL_{back}$, na pasta \texttt{app/front}. A integração com a linha de comando ainda não foi feita, assim como o \emph{Lexer} e o\emph{Parse} da linguagem, de forma que a execução desse módulo precisa alterar o código associado a função \texttt{main}. Com a introdução de \emph{Lexer} e \emph{Parse} para a linguagem, uma forma de executar o programa de forma mais direta e com opções será adicionado.