...
\begin{enumerate}
	...
	\item $\KW{UseAfterFree}$, $\KW{FreeMemoryNotOnHeap}$, $\KW{PartialFree}$ e $\KW{Double Free}$ são falhas temporais (\autoref{sec:mem-error:temporal}), sendo uso de uma região liberada de memória para o primeiro e corrupção da estrutura alocadora por diversos meios para os demais.
...
A notação para descrever as funções neste projeto é similar à de Haskell \cite{HASKELL}, com casamento de padrões nos argumentos das funções e definições tipicamente recursivas. Para a \autoref{fig:def:pop}, as Equações \ref{fig:def:pop1} e \ref{fig:def:pop2} definem as funções $\FN{pop}_{stack}(p)$ e $\FN{pop}_{func}(p)$ com base na função $\FN{pop}(p, v)$, usando como valor inicial de $v$ os seus elementos subscritos. Nas Equações \ref{fig:def:pop3} e \ref{fig:def:pop4} realiza-se a lógica. Se os argumentos casarem o padrão $h : t$, nesse contexto sendo que a lista pode ser dividida entre o elemento no fim $(v, \_)$ e no restante $t$, então executa-se a função definida pela \autoref{fig:def:pop3}. Se a lista for vazia, representada por $[]$, realiza-se a função definida pela \autoref{fig:def:pop4}. $(v,\_)$ também é um casamento de padrão, que atribui o nome $v$ para o primeiro elemento da dupla, o valor, e descarta o segundo, a chave. Com esses elementos, a \autoref{fig:def:pop3} checa se o valor no fim da lista é igual ao código de controle $c_{ctrl}$, se é, retorna o restante da lista $t$, se não, chama $\FN{pop}(t, c_{ctr})$ recursivamente para o restante da lista. Isso descarta elementos do topo da lista até finalmente descartar o $c_{ctrl}$ ou, pela definição da \autoref{fig:def:pop4}, chegar no final e retorna a lista vazia.
...
\infrule[Run]
    {\St{P, \{\}, []} \to_G^* \St{F_0, g, p}\quad\St{F_0, \{\}} \to_F^* \St{\KW{let}\;()\;e , F}}
...
\infrule[Globals-collect]
    {\FN{newkey}() = k \quad \FN{top}(p) = i}
    {\St{\KW{global} \; x[n]\; G , g, p} \to_G\\ \St{G, g[x \mapsto l^p \{i, k, 0, n\}],(\bot, k)_n : p,}} 
}
...
\infrule[LetFunction-collect]
    {}
    {\St{\KW{let} \; f(\overline{x_i})\; e \; F_0 ,F} \to_F \St{F_0, F[f \mapsto \St{[\overline{x_i}], e}]}}
}
...
\labelRule{\label{rule:escopo}}{
	\infrule[Escopo]
		{}
		{F \vdash \St{\{e\},(a,g), p, m} \to \\ \St{\KW{pop}\; e,(\{\} : a, g),\CD{stack} : p, m}}
}

\labelRule{\label{rule:pop-s}}{
	\infrule[Pop-s]
		{F \vdash \St{e,a, p, m} \to \St{e',a', p', m'}}
		{F \vdash \St{\KW{pop}\;e,a, p, m} \to \St{\KW{pop}\;e',a', p', m'}} 
}

\labelRule{\label{rule:pop}}{
	\infrule[Pop]
		{\FN{pop}_{stack}(p) = p'}
		{F \vdash \St{\KW{pop}\;v,(frame : a, g), p, m} \to\\\St{v,(a, g), p', m}}
}
...
\labelRule{\label{rule:let}}{
	\infrule[Let]
		{\FN{top}(p) = i \andalso \FN{newkey}() = k \andalso l = l^p\{i, k, 0, n\}}
		{F \vdash \St{\KW{let}\; x[n],(fr : a, g), p, m} \to\\ \St{l, (fr[x \mapsto l] : a, g),(\bot, k)_n : p, m}} 
}
...
\labelRule{\label{rule:atribui-deref-pilha-obe}}{
	\infrule[Atribui-deref-pilha-obe]
		{0 > o \ge s}
		{F\vdash\St{\text{*}l^p\{i, k, o, s\} := v, a, p, m} \to \\ \St{\KW{panic}\;\KW{OutOfBoundsWrite}, a, p, m}} 
}

\labelRule{\label{rule:atribui-deref-pilha-uaf}}{
	\infrule[Atribui-deref-pilha-uaf]
		{0 \leq o < s\quad p(i + o) = (v_p, k_p) \quad k \neq k_p }
		{F\vdash\St{\text{*}l^p\{i, k, o, s\} := v, a, p, m} \to\\ \St{\KW{panic}\;\KW{UseAfterFree}, a, p, m}} 
}
...
\labelRule{\label{rule:atribui-deref-mem-npd}}{
	\infrule[Atribui-deref-mem-npd]
		{i = 0}
		{F\vdash\St{\text{*}l^m\{i, k, o, s\} := v, a, p, m} \to\\ \St{\KW{panic}\;\KW{NullPtrDeref}, a, p, m}} 
}

\labelRule{\label{rule:atribui-deref-mem-obe}}{
\infrule[Atribui-deref-mem-obe]
    {i \neq 0 \quad 0 > o \ge s}
    {F\vdash\St{\text{*}l^m\{i, k, o, s\} := v, a, p, m} \to\\ \St{\KW{panic}\;\KW{OutOfBoundsWrite}, a, p, m}} 
}

\labelRule{\label{rule:atribui-deref-mem-uaf}}{
	\infrule[Atribui-deref-mem-uaf]
		{i \neq 0 \quad 0 \leq o < s\quad m(i + o) = (v_m, k_m) \quad k \neq k_m }
		{F\vdash\St{\text{*}l^m\{i, k, o, s\} := v, a, p, m} \to\\ \St{\KW{panic}\;\KW{UseAfterFree}, a, p, m}} 
}
...
\labelRule{\label{rule:malloc-s}}{
	\infrule[Malloc-s]
		{F\vdash\St{e, a, p, m} \to\St{e', a', p', m'}}
		{F\vdash\St{\KW{malloc}(e),a, p, m} \to\St{\KW{malloc}(e'),a', p', m'}}
}  

\labelRule{\label{rule:malloc}}{
	\infrule[Malloc]
		{\FN{loc}(m, n) = i \quad \FN{newkey}() = k \quad l = l^m\{i, k, 0, n\}}
		{F\vdash\St{\KW{malloc}(n),a, p, m} \to\St{l, a, p, m[i_{0..n} \mapsto (\bot,k)]}} 
}

\labelRule{\label{rule:free-ls}}{
	\infrule[Free-ls]
		{F\vdash\St{e_1, a, p, m} \to\St{e_1', a', p', m'}}
		{F\vdash\St{\KW{free}(e_1, e_2),a, p, m} \to\St{\KW{free}(e_1', e_2),a', p', m'}}
}
    
\labelRule{\label{rule:free-rs}}{
	\infrule[Free-rs]
		{F\vdash\St{e, a, p, m} \to\St{e', a', p', m'}}
		{F\vdash\St{\KW{free}(l, e),a, p, m} \to\St{\KW{free}(l, e'),a', p', m'}} 
}

\labelRule{\label{rule:free-mnh}}{
	\infrule[Free-mnh]
		{}
		{F\vdash\St{\KW{free}(l^p, n),a, p, m} \to\\ \St{\KW{panic}\;\KW{FreeMemoryNotOnHeap},a, p, m}}
}

\labelRule{\label{rule:free-null}}{
	\infrule[Free-null] 
		{i = 0}
		{F\vdash\St{\KW{free}(l^m\{i, k, o, s\}, n),a, p, m} \to\\ \St{\KW{panic}\;\KW{FreeMemoryNotOnHeap},a, p, m}}
}

\labelRule{\label{rule:free-partial}}{
	\infrule[Free-parcial] 
		{i \neq 0 \quad o \neq 0 \lor s \neq n}
		{F\vdash\St{\KW{free}(l^m\{i, k, o, s\}, n),a, p, m} \to\\ \St{\KW{panic}\;\KW{PartialFree},a, p, m}}
}

\labelRule{\label{rule:free-double}}{
\infrule[Free-double]
    {i \neq 0 \quad o = 0 \quad s = n \quad m(i) = (v_m, k_m) \quad k \neq k_m}
    {F\vdash\St{\KW{free}(l^m\{i, k, o, s\}, n),a, p, m} \to\\ \St{\KW{panic}\;\KW{DoubleFree},a, p, m}}
}

\labelRule{\label{rule:free}}{
	\infrule[Free]
		{i \neq 0 \quad o = 0 \quad s = n \quad m(i) = (v_m, k_m) \quad k = k_m}
		{F\vdash\St{\KW{free}(l^m\{i, k, o, s\}, n),a, p, m} \to\St{n,a, p, m[i_{0..n} \mapsto (-,0)]}}
}
...
\textbf{Acesso a Valores}: O acesso a valores consiste nas regras de acesso a valores de variáveis \hyperref[rule:var]{(VAR)}, dereferência de valores na pilha \hyperref[rule:deref-pilha]{(DEREF-PILHA)} e na memória \hyperref[rule:deref-mem]{(DEREF-MEM)}. Elas acessam a respectiva memória do valor associado, retornando o valor. 

\labelRule{\label{rule:var-obr}}{
	\infrule[Var-obr]
		{a(x) = l^p\{i, k, o, s\} \quad 0 > o \ge s}
		{F \vdash \St{x, a, p, m} \to\\ \St{\KW{panic}\;\KW{OutOfBoundsRead}, a, p, m}}
}
    
\labelRule{\label{rule:var-uaf}}{
	\infrule[Var-uaf]
		{a(x) = l^p\{i, k, o, s\} \\ 0 \leq o < s\quad p(i + o) = (v_p, k_p) \quad k \neq k_p}
		{F \vdash \St{x, a, p, m} \to \St{\KW{panic}\;\KW{UseAfterFree}, a, p, m}}
}

\labelRule{\label{rule:var-ua}}{
	\infrule[Var-ua]
		{a(x) = l^p\{i, k, o, s\} \\ 0 \leq o < s\quad p(i + o) = (v_p, k_p) \quad k = k_p \quad v_p = \bot}
		{F \vdash \St{x, a, p, m} \to \\ \St{\KW{panic}\;\KW{UninitializedAcess}, a, p, m}} 
}
...
\labelRule{\label{rule:deref-pilha-obr}}{
	\infrule[Deref-pilha-obr]
		{0 > o \ge s}
		{F \vdash \St{\text{*}l^p\{i, k, o, s\}, a, p, m} \to \\ \St{\KW{panic}\;\KW{OutOfBoundsRead}, a, p, m}}
}
    
\labelRule{\label{rule:deref-pilha-uaf}}{
	\infrule[Deref-pilha-uaf]
		{0 \leq o < s\quad p(i + o) = (v_p, k_p) \quad k \neq k_p}
		{F \vdash \St{\text{*}l^p\{i, k, o, s\}, a, p, m} \to\\ \St{\KW{panic}\;\KW{UseAfterFree}, a, p, m}} 
}

\labelRule{\label{rule:deref-pilha-ua}}{
	\infrule[Deref-pilha-ua]
		{0 \leq o < s\quad p(i + o) = (v_p, k_p) \quad k = k_p \quad v_p = \bot}
		{F \vdash \St{\text{*}l^p\{i, k, o, s\}, a, p, m} \to\\ \St{\KW{panic}\;\KW{UninitializedAcess}, a, p, m}}
}
...
\labelRule{\label{rule:deref-mem-null}}{
	\infrule[Deref-mem-null]
		{i = 0}
		{F \vdash \St{\text{*}l^m\{i, k, o, s\}, a, p, m} \to\\ \St{\KW{panic}\;\KW{NullPtrDeref}, a, p, m}}
}

\labelRule{\label{rule:deref-mem-obr}}{
	\infrule[Deref-mem-obr]
		{i \neq 0 \quad 0 > o \ge s}
		{F \vdash \St{\text{*}l^m\{i, k, o, s\}, a, p, m} \to\\ \St{\KW{panic}\;\KW{OutOfBoundsRead}, a, p, m}}
}

\labelRule{\label{rule:deref-mem-uaf}}{
	\infrule[Deref-mem-uaf]
		{i \neq 0 \quad 0 \leq o < s\quad m(i + o) = (v_m, k_m) \quad k \neq k_m}
		{F \vdash \St{\text{*}l^m\{i, k, o, s\}, a, p, m} \to\\ \St{\KW{panic}\;\KW{UseAfterFree}, a, p, m}}
}

\labelRule{\label{rule:deref-mem-ua}}{
	\infrule[Deref-mem-ua]
		{i \neq 0 \quad 0 \leq o < s\quad m(i + o) = (v_m, k_m) \quad k = k_m \quad v_m = \bot}
		{F \vdash \St{\text{*}l^m\{i, k, o, s\}, a, p, m} \to\\ \St{\KW{panic}\;\KW{UninitializedAcess}, a, p, m}} 
}
...
\labelRule{\label{rule:if-s}}{
	\infrule[If-s]
		{F \vdash \St{e_1,a, p, m} \to \St{e_1', a', p', m'}}
		{F \vdash \St{\KW{if}\;(e_1)\; e_2\; \KW{else}\; e_3 ,a, p, m} \to\\ \St{\KW{if}\;(e_1')\; e_2\; \KW{else}\; e_3 ,a', p', m'}}
}

\labelRule{\label{rule:if-true}}{
	\infrule[If-true]
		{\mathcal{N}(n) \neq 0}
		{F \vdash \St{\KW{if}\;(n)\; e_1\; \KW{else}\; e_2 ,a, p, m} \to \St{e_1, a, p, m}} 
}

\labelRule{\label{rule:if-false}}{
	\infrule[If-false]
		{\mathcal{N}(n) = 0}
		{F \vdash \St{\KW{if}\;(n)\; e_1\; \KW{else}\; e_2 ,a, p, m} \to \St{e_2, a, p, m}} 
}
    
\labelRule{\label{rule:while}}{
	\infrule[While]
		{}
		{F \vdash \St{\KW{while}\;(e_1)\; e_2, a, p, m} \to \\ \St{\KW{if}\;(e_1)\; (e_2;\KW{while}\;(e_1)\; e_2)\; \KW{else}\; 0, a, p, m}} 
}
...
\labelRule{\label{rule:callfunc}}{
	\infrule[CallFunc]
		...
		{F \vdash \St{f(\overline{v_i}), a, p, m} \to \\\St{\KW{Fpop}\,\{\ \overline{\KW{let}\;x_i[1]; x_i := v_i;} e\}, (STOP : a, g), \CD{func} : p, m}} 
}
\labelRule{\label{rule:fpop-s}}{
	\infrule[Fpop-s]
		{F \vdash \St{e,a, p, m} \to \St{e',a', p', m'}}
		{F \vdash \St{\KW{Fpop}\;e,a, p, m} \to \St{\KW{Fpop}\;e',a', p', m'}} 
}

\labelRule{\label{rule:fpop}}{
	\infrule[Fpop]
		{\FN{pop}_{func}(p) = p'}
		{F \vdash \St{\FN{Fpop}\; v, (h : a, g), p, m} \to \St{v, (a, g), p', m}} 
}
...
\begin{lstlisting}[language=C, label={lst:c:spacial}, caption={Exemplo de \emph{Incorret-Calculation-of-Buffer-Size} em C.}]
